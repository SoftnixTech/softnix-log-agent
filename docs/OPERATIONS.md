...
Every route except `/` and `/healthz` requires authentication — even when bound to `127.0.0.1`. If `web.auth_token` is left unset, the agent generates one on startup into `<data_dir>/web-token` (mode `0600`); the GUI reads it automatically on first visit via a `#token=…` URL fragment (stored in the browser's session storage after that) or prompts for it, pointing at that file. API clients send `Authorization: Bearer <token>` (or `X-Auth-Token`). Binding beyond localhost (`web.bind: 0.0.0.0`) **without** `web.auth_token` set is a hard startup refusal, not a warning — the agent will not start. Prefer a firewall/SSH tunnel over public exposure; the GUI is plain HTTP.
...
- `GET /metrics` — requires the same bearer token as every other route except `/` and `/healthz`. Prometheus-style text: `agent_events_received_total`, `agent_events_sent_total`, `agent_events_failed_total`, `agent_events_dropped_total`, `agent_errors_total`, `agent_queue_events{destination=…}`, `agent_queue_bytes{…}`, `agent_queue_dropped_total{…}`, `agent_queue_full{output=…}` (1 when that destination's queue is full **and** its policy is `block`, 0 otherwise — same policy-aware gating as `/healthz`), `agent_router_shed_total{destination=…}` (events shed because that destination's in-memory router channel was momentarily full — see [Backpressure and the full-queue policy](#backpressure-and-the-full-queue-policy)), `agent_queue_corrupt_records_total{destination=…}` (records skipped because they failed CRC or would not decode — should stay at 0; a rising counter means on-disk corruption, see `/api/buffer`'s `corrupt_records` field for the same figure), `agent_output_healthy{…}`, `agent_input_events_total{input=…}` (per-input event count — since an input can no longer die outright, a flat rate on a channel that should be producing events is the signal it's wedged), `agent_uptime_seconds`.
- `GET /api/status|inputs|outputs|buffer|logs|about` — JSON equivalents used by the GUI.
- `GET /api/update/status` — read-only self-update check (only makes a network call when `update.check_url` is configured; see [`CONFIGURATION.md`'s `update` section](CONFIGURATION.md#update)). Never applies anything.

Alert suggestions: `agent_output_healthy == 0` for >5 min; `agent_queue_bytes` approaching `buffer.max_size_mb`; `agent_queue_full == 1` for any destination (collection may be stalled for that destination — see below); `agent_events_dropped_total` increasing; `agent_router_shed_total` increasing for a destination whose queue is *not* full (brief lock contention, not a stalled destination — worth watching but not urgent on its own); `agent_queue_corrupt_records_total` increasing at all (silent on-disk queue corruption — investigate the destination's data directory and filesystem); `rate(agent_input_events_total[1h]) == 0` for an input expected to be active (a channel wedged in a permanent retry loop looks identical to a healthy one on `/healthz`, since it never exits — this is the only external signal that it has stopped producing); `agent_uptime_seconds` resets (crash loop).
